...
INSERT INTO members (regDate, loginId, loginPw, `name`)
...
ALTER TABLE members ADD UNIQUE INDEX(`loginId`);
...
ALTER TABLE members DROP INDEX `loginId`;
...
ALTER TABLE articles DROP COLUMN `writerName`;
...
SELECT articles.id, articles.memberId, members.id AS "회원테이블_번호"
FROM articles
INNER JOIN `members`;

# 조인 완성, ON 까지 추가
SELECT a.id, a.title, a.memberid, m.`name` FROM articles a
INNER JOIN members m
ON a.memberId = m.id
ORDER BY a.id;
